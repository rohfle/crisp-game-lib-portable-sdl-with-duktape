...
var enemyPattern = [[1, 1, 0.65], [1, 0.8, 2], [1.6, 0.6, 2], [1, 2, 1.8]];
function update() {
  if (!ticks) {
    hoppings = [{
      pos: vec(99, 60),
  ...
  color("blue");
  rect(0, 90, 199, 9);
  var isWeakApp = powerTicks > 60;
  enemyAppTicks -= isWeakApp ? 3 : 1;
  if (enemyAppTicks < 0) {
    var x = rnd() < 0.5 ? -5 : 205;
    var t = enemyPattern[rndi(enemyPattern.length)];
    hoppings.push({
      pos: vec(x, 89),
      vel: vec(),
      vx: x < 99 ? 1 : -1,
  ...
  powerAppTicks--;
  if (powerAppTicks < 0) {
    var _x = rnd() < 0.5 ? -5 : 205;
    var _t = isFirstPower ? [0.88, 1.25, 1.25] : enemyPattern[rndi(enemyPattern.length)];
    isFirstPower = false;
    hoppings.push({
      pos: vec(_x, 89),
      vel: vec(),
      vx: _x < 99 ? 1 : -1,
      hop: vec(baseHop.x * _t[0] * 0.8, baseHop.y * _t[1] * 0.8),
      grv: baseGrv * _t[2] * 0.8,
      hopTicks: 0,
      type: "power"
    });
  ...
  hoppings = hoppings.filter(function (h) {
    ...
    if (h.hopTicks > 0) {
      ...
      if (h.hopTicks <= 0) {
        ...
        var r = h.hopTicks < 5 ? 1 - h.hopTicks / 5 : (h.hopTicks - 5) / 5;
        color("black");
        box(h.pos.x, h.pos.y - 2 * r, 2, 4 * r);
        box(h.pos.x, h.pos.y - 5 * r, 6, 2 * r);
        var _c = drawHopping(h, r);
        return checkCollision(h, _c);
      }
    }
    h.pos.add(h.vel);
  ...
  function checkCollision(h, c) {
    if (h.type === "enemy") {
      ...
      if (c.o) {
        ...
        color("yellow");
        particle(h.pos, 29, 3);
        powerTicks = floor(300 / sqrt(difficulty) * sqrt(sqrt(hoppings.length)));
        hoppings.forEach(function (h) {
          if (h.type !== "player") {
            h.vel.x *= -1;
            h.vx *= -1;
          }
  ...
  function drawHopping(h, r) {
    color(powerTicks > 0 && h.type === "enemy" && !(powerTicks < 60 && powerTicks % 15 > 7) ? "blue" : colors[h.type]);
    var c;
