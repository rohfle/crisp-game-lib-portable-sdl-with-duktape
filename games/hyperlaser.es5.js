...
function update() {
  ...
  color("red");
  var sp = input.pos.clamp(1, 99, 1, 99);
  char("b", sp);
  var ta = player.pos.angleTo(sp);
  if (input.isPressed) {
    ...
    var x = clamp(input.pos.x, 7, 93);
    var s = sqrt(difficulty) * 0.3;
    if (player.pos.x < x - 1) {
      player.pos.x += s;
  ...
  remove(lasers, function (l) {
    l.pos.y += scr;
    var pp = vec(l.pos);
    color("transparent");
    l.pos.add(l.vel);
    var isColliding = false;
    if (bar(l.pos.x, pp.y, 5, 2, l.vel.angle).isColliding["char"].a || l.pos.x < 5 && l.vel.x < 0 || l.pos.x > 95 && l.vel.x > 0) {
      l.vel.x *= -1;
      isColliding = true;
    }
    ...
    if (!isColliding && bar(l.pos.x, l.pos.y, 5, 2, l.vel.angle).isColliding["char"].a) {
      l.vel.x *= -1;
      l.vel.y *= -1;
      isColliding = true;
    }
    ...
    l.ticks++;
    var c = bar(l.pos, 5, 2, l.vel.angle).isColliding;
    if (c["char"].a) {
      return true;
    }
  ...
  nextEnemyDist--;
  if (nextEnemyDist < 0) {
    var shotInterval = rnd(60, 120) / sqrt(difficulty);
    enemies.push({
      pos: vec(rnd(10, 90), -9),
      angle: PI / 2,
      targetAngle: PI / 2,
  ...
  remove(enemies, function (e) {
    ...
    color("yellow");
    var c = bar(e.pos, 2, 3, e.angle).isColliding;
    var p = vec(e.pos);
    color("light_black");
    ...
    bar(p, 4, 1, e.angle);
    e.shotTicks--;
    var hasShield = e.shotTicks > 0 && e.pos.y < 70;
    if (hasShield) {
    ...
    if (isMoving && (c["char"].a || e.turnTicks < 0 || e.pos.x < 9 || e.pos.x > 90)) {
      e.pos.set(pp);
      e.targetAngle = wrap(e.targetAngle + (rndi(2) * 2 - 1) * PI / 2, -PI, PI);
  ...
  remove(shots, function (s) {
    s.pos.y += scr;
    s.pos.add(s.vel);
    var c = char("c", s.pos).isColliding;
