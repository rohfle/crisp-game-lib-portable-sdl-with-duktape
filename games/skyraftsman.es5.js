...
function updateRaftsman() {
  ...
  if (raftsman.isJumping) {
    ...
    raftsman.pos.set(raftsman.currentLog.pos).addWithAngle(raftsman.currentLog.angle, raftsman.currentLog.size / 2);
  }
  if (raftsman.pos.x < 3 && raftsman.vel.x < 0 || raftsman.pos.x > 97 && raftsman.vel.x > 0) {
...
function updateLogs() {
  remove(logs, function (log) {
    ...
    if (log.pos.x < log.size / 2 && log.vel.x < 0 || log.pos.x > 100 - log.size / 2 && log.vel.x > 0) {
      log.vel.x = -log.vel.x;
    }
...
function drawGame() {
  ...
  logs.forEach(function (log) {
    if (log.pos.y > 100 + log.size / 2) {
      ...
      color("yellow");
      if (arc(log.pos, log.size / 2, 3, log.angle, log.angle + 2 * PI).isColliding.rect.red && raftsman.isJumping) {
        play("click");
