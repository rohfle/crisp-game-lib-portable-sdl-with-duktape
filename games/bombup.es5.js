...
function update() {
  ...
  player.vel.y += 0.02;
  if (player.pos.x < 7 && player.vel.x < 0 || player.pos.x > 93 && player.vel.x > 0) {
    player.vel.x *= -0.7;
  ...
  if (input.isJustPressed) {
    if (bomb == null) {
      ...
      var d = bomb.pos.distanceTo(player.pos);
      var a = bomb.pos.angleTo(player.pos);
      player.vel.addWithAngle(a, 20 / d);
      bomb = undefined;
    }
  }
  if (bomb != null) {
    if (bomb.pos.x < 7 && bomb.vel.x < 0 || bomb.pos.x > 93 && bomb.vel.x > 0) {
      bomb.vel.x *= -0.7;
    }
    bomb.vel.y += 0.15;
    bomb.vel.mul(0.98);
  ...
  if (explosion != null) {
    explosion.ticks++;
    var r = sin(explosion.ticks * 0.15) * 25;
    if (r < 0) {
      explosion = undefined;
    } else {
      color("light_red");
  ...
  if (nextRockX < 7 && nextRockVelX < 0 || nextRockX > 93 && nextRockVelX > 0) {
    nextRockVelX *= -0.7;
    nextRockX = clamp(nextRockX, 7, 93);
  }
  if (nextRockTicks < 0) {
    var isBottom = rnd() > (player.pos.y > 30 ? 0.05 : 0.9);
    rocks.push({
