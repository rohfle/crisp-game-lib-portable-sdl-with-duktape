...
function update() {
  ...
  if (target.initTicks > 0) {
    ...
    color("red");
    var c = arc(target.pos, target.size).isColliding.rect;
    if (c.green) {
      play("coin");
  ...
  remove(walls, function (w) {
    color("".concat(w.isFixed ? "light_" : "").concat(w.angle === 0 ? "blue" : "cyan"));
    var wd = w.isFixed ? 100 : 20;
    var c = box(w.pos, w.angle === 0 ? 2 : wd, w.angle === 0 ? wd : 2).isColliding.rect;
