...
function update() {
  ...
  remove(enemies, function (e) {
    ...
    var ms = sqrt(difficulty) * 0.1;
    e.vel.x = clamp(e.vel.x + (ship.pos.x - e.pos.x) * 0.0002 * sqrt(difficulty), -ms, ms);
    if (e.pos.y < 10) {
      e.vel.y = 0;
    }
    e.pos.add(e.vel);
    if (char(abs(e.vel.x) < 0.1 ? "b" : "a", e.pos, {
      mirror: {
        x: e.vel.x < 0 ? -1 : 1,
        y: abs(e.vel.x) < 0.1 ? -1 : 1
      }
  ...
  remove(bombs, function (b) {
    ...
    var ty = b.pos.y - (10 - b.count) * 2;
    if (b.count < 1) {
      ty -= (1 - b.count) * 50;
    }
    if (text("".concat(ceil(b.count)), b.pos.x, ty).isColliding.rect.red) {
      b.count = 0;
    }
    color("purple");
    particle(b.pos.x, b.pos.y - (10 - b.count) / 2, 0.3, (10 - b.count) * 0.1, -PI / 2, PI / 3);
    var pc = ceil(b.count);
    b.count -= 1 / 60;
    if (pc != ceil(b.count)) {
      play("hit");
  ...
  color("black");
  if (input.isJustPressed || ship.pos.x > 99 && ship.targetVx > 0 || ship.pos.x < 1 && ship.targetVx < 0) {
    play("select");
    ship.targetVx *= -1;
  }
  ship.vx += (ship.targetVx - ship.vx) * 0.1;
  ship.pos.x += ship.vx * sqrt(difficulty);
  var c = char(abs(ship.vx) < 0.5 ? "b" : "a", ship.pos, {
    mirror: {
      x: ship.vx < 0 ? -1 : 1
    }
  }).isColliding;
