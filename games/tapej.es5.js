...
function update() {
  ...
  color("light_yellow");
  var j = tapes.length > 0 ? tapes[0].from : head.from;
  box(j.x - 1, j.y - 1, 5, 5);
  color("purple");
  box(head.to, 3, 3);
  scr = difficulty * 0.1;
  ...
  if (nextRectDist < 0) {
    ...
    rects.push({
      x: 200,
      size: vec(w, rnd(10, 50))
    });
    nextRectDist = w + rnd(20, 70);
  }
  color("light_black");
  rect(0, groundY, 200, 100 - groundY);
  remove(rects, function (r) {
    if (rect(r.x, 90, r.size.x, -r.size.y).isColliding.rect.purple && head.type === "ground") {
      var to = vec(r.x - 1, groundY - 1);
      tapes.push({
        from: vec(head.from),
        to: to
      });
  ...
  color("red");
  remove(fires, function (f) {
    f.pos.add(f.vel);
    f.pos.x -= scr;
    var cl = box(f.pos, f.size, f.size).isColliding.rect;
    particle(f.pos.x + rnds(f.size / 2), f.pos.y + rnds(f.size / 2), rnd() * f.size / 2, -f.vel.length, f.vel.angle, 0.3);
