...
function update() {
  ...
  if (nextNodeDist < 0) {
    ...
    nodes.forEach(function (n) {
      if (n.color === "black" && abs(nn.pos.x - n.pos.x) / 2 + abs(nn.pos.y - n.pos.y) < 30 && rnd() < 0.7) {
        lines.push({
          node1: nn,
          node2: n,
  ...
  remove(lines, function (l) {
    var d = l.node1.pos.distanceTo(l.node2.pos) - l.length;
    var a = l.node1.pos.angleTo(l.node2.pos);
  ...
  function checkRedNodes() {
    ...
    var _loop = function _loop() {
      var isAddingRed = false;
      lines.forEach(function (l) {
        if (l.node1.color === "black" || l.node1.color === "red" || l.node2.color === "black" || l.node2.color === "red") {
