...
function update() {
  ...
  var scrollSpeed = sd;
  cleanerHead.position.set(cleaner.position).addWithAngle(sin(ticks * 0.1 * sd) - PI / 2, cleaner.length);
  ...
  if (input.isPressed) {
    ...
    cleaner.position.x += (cleanerHead.position.x - cleaner.position.x) * 0.005 * cleaner.length * sd;
  ...
  color("light_black");
  remove(dusts, function (d) {
    d.position.y += scrollSpeed;
    var c = bar(d.position, d.size.x, d.size.y, d.angle).isColliding.rect;
  ...
  color("blue");
  if (arc(cleaner.position.x, cleaner.position.y + 9, 3, 5).isColliding.rect.red) {
    play("explosion");
    end();
  }
}
