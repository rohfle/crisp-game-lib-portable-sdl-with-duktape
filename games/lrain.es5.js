...
function update() {
  ...
  if (nextLaserTicks < 0) {
    play("laser");
    var x = rnd(99);
    var vy = sqrt(difficulty) * 3 * (rndi(2) * 2 - 1);
    var width = rnd(3, 9);
    ...
    var tr = abs(cos(laserDurationAngle));
    if (tr < 0.15) {
      tr = 5;
    }
  ...
  remove(lasers, function (l) {
    if (l.pos.y < 0 || l.pos.y > 99) {
      l.ticks += difficulty;
      var w = sin(l.ticks * 0.1) * l.width * 2;
      if (w <= 0) {
        return true;
      }
      w = clamp(w, 0, l.width - 1);
      var sy = abs(l.vy) * rnd(3, 5);
      times(floor(w), function () {
        laserBoxes.push({
          pos: vec(l.pos.x + rnds(w), l.pos.y + (50 - l.pos.y) * 2 + (rnd(9) + sy / 2) * (l.vy > 0 ? -1 : 1)),
          vy: l.vy * 3,
          size: vec(3, sy),
  ...
  var isHit = false;
  remove(laserBoxes, function (b) {
    b.pos.y += b.vy;
    color(b.color);
    var c = box(b.pos, b.size).isColliding.rect;
    if (c.white) {
      play("hit");
      addScore(multiplier);
      color("black");
