...
function update() {
  ...
  player.pos.x = vine.centerX + Math.pow(Math.sin((vine.ticks + player.pos.y * vine.bending) * vine.frequency / 2), 2) * vine.amplitude * 2 - vine.amplitude;
  vine.ticks++;
  ...
  for (var y = 0; y <= 100; y += 5) {
    var x = vine.centerX + Math.pow(Math.sin((vine.ticks + y * vine.bending) * vine.frequency / 2), 2) * vine.amplitude * 2 - vine.amplitude;
    box(x, y, 2, 5);
  }
  ...
  if (coin == null) {
    var vx = rnds(0.18, 0.25) * difficulty;
    coin = {
  ...
  color("yellow");
  coin.pos.x += coin.vx;
  var c = char("d", coin.pos).isColliding["char"];
  if (c.a || c.b) {
    play("coin");
