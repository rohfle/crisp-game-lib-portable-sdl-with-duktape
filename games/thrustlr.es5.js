...
function update() {
  ...
  var scr = difficulty * 0.2;
  nextRectDist -= scr;
  if (nextRectDist < 0) {
    var size = addRect();
    nextRectDist = rnd(10, size);
  }
  var tx = -(clamp(input.pos.x, 0, 100) - 50);
  remove(rects, function (r) {
    r.pos.y += scr;
    if (r.pos.y - r.size < shipY && r.pos.y + r.size > shipY) {
      var d = r.pos.x - ship.x;
      if (d * tx < 0) {
        r.vx -= tx / (abs(d) - (r.size - abs(r.pos.y - shipY))) * 0.2 / r.size * difficulty;
      }
      r.vx *= 0.98;
    }
    r.pos.x += r.vx;
  ...
  var c1 = line(ship.x - 1, shipY - 2, ship.x - 2, shipY + 2, 2).isColliding.rect.black;
  var c2 = line(ship.x + 1, shipY - 2, ship.x + 2, shipY + 2, 2).isColliding.rect.black;
  if (c1 || c2) {
    play("explosion");
    end();
  }
  function addRect() {
    ...
    var t = rnd() < 0.3;
    var size = t ? rnd(7, 12) : rnd(20, 30);
    var x = t ? rnd(10, 90) : 50 + rnds(40, 60);
    rects.push({
      pos: vec(x, y - size),
      vx: 0,
      size: size
