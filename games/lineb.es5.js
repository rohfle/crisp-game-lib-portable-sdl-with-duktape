...
function update() {
  ...
  color("light_black");
  pointsHistory.forEach(function (ph, i) {
    if (i % 9 === 8) {
  ...
  if (input.isJustPressed) {
    play("laser");
    var cp = vec(points[0].pos).add(points[1].pos).div(2);
  ...
  if (line(points[0].pos, points[1].pos).isColliding.rect.yellow) {
    ...
    for (var _i = 0; _i < 4; _i++) {
      var p = vec(gold.pos).addWithAngle(_i * PI / 2, gold.radius);
