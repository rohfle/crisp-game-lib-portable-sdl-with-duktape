...
function update() {
  ...
  var scr = sqrt(difficulty) * 0.05 + (player.pos.y < scrY ? (scrY - player.pos.y) * 0.1 : 0);
  scrY += (80 - scrY) * 0.001;
  nextShieldDist -= scr;
  if (nextShieldDist < 0) {
    play("hit");
    var tx = rndi(10) * 10 + 5;
    shields.push({
      pos: vec(tx < 50 ? 0 : 99, 15),
      tx: tx
    });
    nextShieldDist += rnd(9, 36);
  }
  remove(shields, function (s) {
    s.pos.y += scr;
  ...
  if (laserCount > 10) {
    ...
    if (laserCount > 199) {
      play("coin");
      addScore(ceil(player.dist * sqrt(player.dist) * sqrt(difficulty) * 0.1), player.pos);
      resetLaser();
    }
  } else {
  ...
  color("black");
  if (char(addWithCharCode("a", floor(player.ticks / 20) % 2), player.pos, {
    mirror: {
      x: player.vx
    }
