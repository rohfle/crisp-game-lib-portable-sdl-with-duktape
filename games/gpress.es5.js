...
function update() {
  ...
  if (press.vw !== 0) {
    ...
    if (press.y < pressHeight / 2 && press.vy < 0 || press.y > waterY - pressHeight / 2 && press.vy > 0) {
      press.vy *= -1;
    }
  ...
  remove(bubbles, function (b) {
    ...
    if (box(b.pos, b.size).isColliding.rect.red) {
      if (b.size > press.width) {
        ...
        b.pos.x = b.pos.x < 50 ? 50 - press.width / 2 + b.size / 2 : 50 + press.width / 2 - b.size / 2;
      }
    }
    return b.pos.y < -b.size / 2;
  });
  color("light_purple");
  remove(drops, function (d) {
  ...
  if (press.vw === 0 && drops.length === 0 && waterY >= 100 && targetWaterY >= 100) {
    play("lucky");
    end();
  }
}
