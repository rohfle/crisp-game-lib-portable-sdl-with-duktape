...
var JUMP_HEIGHT = 30;
var SCROLL_SPEED = 1;
var SWORD_ROTATION_SPEED = 0.3;
var MAX_SWORD_LENGTH = 30;
...
function update() {
  ...
  if (player.isJumping) {
    ...
    var jumpProgress = player.jumpTicks / JUMP_DURATION;
  ...
  box(player.pos, 4, 8);
  color("cyan");
  var swordTip = vec(player.swordLength, 0).rotate(player.swordAngle);
  line(player.pos.x, player.pos.y - 4, player.pos.x + swordTip.x, player.pos.y - 4 + swordTip.y, player.isJumping ? 5 : 2);
  ...
  remove(enemies, function (e) {
    ...
    color("red");
    var isCollidingWithSword = box(e.pos, e.size).isColliding.rect.cyan;
  ...
  ground.segments.forEach(function (segment, index) {
    segment.x -= SCROLL_SPEED * sqrt(difficulty);
    color("green");
    rect(segment.x, GROUND_Y, segment.width - 2, 20);
    if (segment.x + segment.width < 200 && index === ground.segments.length - 1) {
      if (rnd() < gapFrequency) {
        var gapWidth = rnd(15, 30);
  ...
  remove(ground.segments, function (segment) {
    if (segment.x + segment.width < 0) {
      return true;
    }
    if (!player.isJumping && player.pos.x > segment.x && player.pos.x < segment.x + segment.width) {
      player.pos.y = GROUND_Y;
    }
    return false;
  });
  var isOverGap = !ground.segments.some(function (segment) {
    return player.pos.x >= segment.x && player.pos.x <= segment.x + segment.width;
  });
