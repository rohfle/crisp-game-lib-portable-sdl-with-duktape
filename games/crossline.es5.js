...
function update() {
  ...
  while (nextPointDist < 0) {
    var vy = pointCount % 2 === 0 ? -1 : 1;
    var pc = floor(pointCount / 2) % 7;
    points.push({
      pos: vec(pc == 3 ? rnd(10, 30) : pc === 6 ? rnd(70, 90) : rnd(10, 90), vy > 0 ? -nextPointDist - 19 : 119 + nextPointDist),
      vy: vy
    });
    nextPointDist += 4;
    pointCount++;
  ...
  remove(points, function (p) {
    ...
    p.pos.y += scr * p.vy;
    color("transparent");
    var c = box(p.pos, 4).isColliding.rect;
    if (p !== selectedPoint && !isSelected && c.light_red) {
      ...
      if (selectedPoint == null) {
        ...
        color("red");
        var a = p.pos.angleTo(selectedPoint.pos);
    ...
    var my = p.vy > 0 ? p.pos.y : 100 - p.pos.y;
    if (my > pointMaxY) {
      pointMaxY = my;
    }
  });
  ...
  function addLineParticle(from, to) {
    ...
    var p = vec(from);
    var o = vec(7).rotate(from.angleTo(to));
    times(floor(d / 7), function () {
      particle(p, 5, 0.5);
      p.add(o);
    });
  }
