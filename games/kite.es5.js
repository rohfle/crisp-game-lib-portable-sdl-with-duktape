...
function update() {
  if (!ticks) {
    ...
    nextCoinDist = 0;
    spikes = [];
    spikeAngles = times(2, function (i) {
      return {
        yAngle: i * PI / 2,
        hAngle: i * PI
      };
  ...
  var scr = kite.pos.x > 60 ? (kite.pos.x - 60) * 0.1 : 0;
  nextSpikeDist -= scr;
  if (nextSpikeDist < 0) {
    addScore(1);
    spikeAngles.forEach(function (a, i) {
      var height = sin(a.yAngle) * 9 + sin(a.hAngle) * 20 - 10;
      a.yAngle += rnd(sd) * 0.2;
      a.hAngle += rnd(sd) * 0.4;
      if (height > 0) {
        spikes.push({
  ...
  color("light_black");
  var p = vec(kite.pos.x - 3, kite.pos.y + 3);
  ...
  color("black");
  if (char(addWithCharCode("a", floor(player.ticks / 15) % 2), player.pos).isColliding.rect.red) {
    play("explosion");
    end();
  }
  var d = player.pos.distanceTo(kite.pos);
  if (d > stringDist) {
    var a = player.pos.angleTo(kite.pos);
  ...
  color("light_cyan");
  remove(winds, function (w) {
    w.pos.add(w.vel);
    var c = char("c", w.pos).isColliding.rect;
    if (c.blue) {
      play("hit");
      kite.vel.add(w.vel);
      return true;
  ...
  remove(coins, function (c) {
    c.pos.add(c.vel);
    c.pos.x -= scr;
    var cl = char("d", c.pos).isColliding;
    if (cl.rect.red) {
      return true;
    }
