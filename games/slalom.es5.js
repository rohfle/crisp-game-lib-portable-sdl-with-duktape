...
function update() {
  ...
  if (rockAppDist < 0) {
    var x = rnd() < 0.5 ? rndi(-5, 5) : rndi(95, 105);
    rocks.push({
      p: vec(x, -9),
      s: vec(rndi(9, 19), rndi(5, 9) * 2)
    });
    rockAppDist += 10;
  }
  color("purple");
  ...
  if (tw !== targetWall) {
    targetWall = tw;
    var s = floor(100 / (sqrt(minDist) + 1)) - 15;
    if (s > 0) {
      play("coin");
      addScore(s, pos);
    }
  ...
  bar(vec(pos).add(vec(-2, -2).rotate(angle)), 1, 2, angle);
  color("blue");
  var pc = bar(pos, 4, 3, angle).isColliding.rect;
  ...
  var wp = vec(w.side === 1 ? 99 - w.length : w.length, w.y);
