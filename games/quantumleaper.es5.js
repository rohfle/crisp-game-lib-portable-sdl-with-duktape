...
function updatePlayerPosition() {
  ...
  player.pos.y = universeLanes[player.currentUniverse].y + sin(player.waveOffset) * waveAmplitude;
}
function handlePlayerInput() {
  if (input.isJustPressed) {
    player.currentUniverse = wrap(player.currentUniverse + (sin(player.waveOffset) > 0 ? 1 : -1), 0, numUniverses);
  }
}
function updateAndDrawUniverseLanes() {
  ...
  universeLanes.forEach(function (lane, i) {
...
function updateAndDrawAntiParticles() {
  color("purple");
  remove(antiParticles, function (ap) {
...
function checkCollisions() {
  var isColliding = box(player.pos, 4).isColliding.rect.purple;
  if (isColliding) {
    play("explosion");
    end();
  }
...
function spawnAntiParticles() {
  if (rnd() < 0.02 * difficulty) {
    play("click");
    var universe = rndi(0, numUniverses);
    antiParticles.push({
