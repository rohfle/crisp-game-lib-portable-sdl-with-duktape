...
function update() {
  ...
  for (var _i3 = 5; _i3 < 10; _i3++) {
    var _l = lines[_i3];
    _l.pos.x = wrap(_l.pos.x + scr.x, 0, 100);
    box(_l.pos, _l.size);
  }
  pos.add(scr);
  color("blue");
  ...
  arrows = arrows.filter(function (a) {
    ...
    color("red");
    var c = bar(a.pos, 5, 2, a.angle).isColliding.rect;
    if (c.blue || c.green) {
      if (!checkCollision(a)) {
        play("powerUp");
    ...
    color("yellow");
    if (bar(vec(a.pos).addWithAngle(a.angle, 4), 2, 2, a.angle + PI / 2).isColliding.rect.blue) {
      if (!checkCollision(a)) {
        play("lucky");
        end();
      }
  ...
  if (nextArrowTicks < 0) {
    ...
    for (var _i4 = 0; _i4 < 9; _i4++) {
      ...
      if (abs(wrap(pos.x - p.x, -50, 50)) + abs(wrap(pos.y - p.y, -50, 50)) > 36) {
        isApp = true;
        break;
      }
    }
  ...
  color("black");
  text("+".concat(floor(multiplier)), 3, 97);
