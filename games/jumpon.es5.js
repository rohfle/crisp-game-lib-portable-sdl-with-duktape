...
function update() {
  ...
  remove(holes, function (h) {
    ...
    var ai = h.animTicks < 21 ? [1, 2, 1, 0, 3, 4, 3][floor(h.animTicks / 3)] : 0;
  ...
  if (player.state === "walk") {
    ...
    var c = char(addWithCharCode("a", floor(ticks / 10) % 2), player.pos).isColliding["char"];
    if (c.e) {
      play("explosion");
      end();
    }
    ...
    player.pos.x += difficulty - scr;
    var o = player.hole.x - player.pos.x;
    ...
    player.pos.x += difficulty - scr;
    var _o3 = player.pos.x - player.hole.x;
    player.pos.y += _o3 < 4 ? -1 : 1;
    if (_o3 >= 8) {
      player.state = "walk";
  ...
  remove(dots, function (d) {
    d.pos.x -= scr;
    var c = char("g", d.pos).isColliding["char"];
    if (c.a || c.b) {
      play("coin");
