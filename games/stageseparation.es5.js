...
function update() {
  ...
  var altitude = -rocket.pos.y;
  addScore(-rocket.vel.y * 0.1);
  var targetCameraY = rocket.pos.y - 130 * zoom;
  cameraY += clamp((targetCameraY - cameraY) * 0.1, -99, 0);
  zoom = clamp(1 + altitude / 9999, 1, 3);
  color("black");
  stars.forEach(function (star) {
    var screenX = (star.x - 50) / zoom + 50;
    var screenY = (star.y - cameraY) % 1000 / zoom;
  ...
  if (input.isJustPressed && rocket.currentStage > 2) {
    ...
    separatedStages.push({
      pos: vec(rocket.pos.x, rocket.pos.y + rocket.stageHeight * (rocket.currentStage - 1)),
  ...
  var rocketScreenPos = worldToScreen(rocket.pos);
  rect(rocketScreenPos.x - 5 / zoom, rocketScreenPos.y, 10 / zoom, rocket.stageHeight * rocket.currentStage / zoom);
  if (rocket.stageBurnTime > 0) {
    particle(rocketScreenPos.x, rocketScreenPos.y + rocket.stageHeight * rocket.currentStage / zoom, {
      count: 1,
  ...
  color("purple");
  remove(separatedStages, function (stage) {
    stage.vel.y += 0.1;
    stage.pos.add(stage.vel);
    var stageScreenPos = worldToScreen(stage.pos);
    rect(stageScreenPos.x - 5 / zoom, stageScreenPos.y, 10 / zoom, stage.height / zoom);
    return stage.pos.y > cameraY + 200 * zoom;
  });
  nextDebrisSpawn += rocket.vel.y * difficulty;
  if (nextDebrisSpawn < 0) {
    var side = rndi(0, 2);
    var spawnX = side ? 50 + 70 * zoom : 50 - 70 * zoom;
    debris.push({
  ...
  color("light_black");
  remove(debris, function (d) {
    d.pos.add(vec(d.vel).mul(difficulty));
    var debrisScreenPos = worldToScreen(d.pos);
    var debrisCollision = arc(debrisScreenPos.x, debrisScreenPos.y, d.size / zoom).isColliding.rect;
