...
function update() {
  ...
  explosions = explosions.filter(function (e) {
    var a = e.a;
    var r = e.t < 20 ? e.t * 0.5 : 20 * 0.5 - (e.t - 20);
    var s = e.t === 0 ? 10 : r + 3;
    times(5, function (i) {
  ...
  if (bombAppTicks < 0) {
    var p = vec(rnd(80), 0);
    var v = vec(rnd(20, 70), 70).sub(p).div(500 / rnd(1, difficulty));
    bombs.push({
      p: p,
      v: v
    });
  ...
  bombs = bombs.filter(function (b) {
    b.p.add(b.v);
    color("red");
    var bc = box(b.p, 5, 5).isColliding.rect;
