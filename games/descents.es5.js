...
function update() {
  ...
  color("black");
  var c = char("a", ship.pos).isColliding.rect;
  if (c.black) {
    if (ship.vel.y > 1) {
      play("explosion");
      end();
  ...
  color("black");
  text("".concat(-ship.up, "m/s"), 76 - (-ship.up > 9 ? 6 : 0), 10);
  char("c", 97, 9);
  color(!input.isPressed ? "yellow" : "cyan");
  char("b", 60, 17);
  color("black");
  text("".concat(ship.down, "m/s"), 76 - (ship.down > 9 ? 6 : 0), 17);
  char("c", 97, 16);
  text("NEXT", 70, 24);
  color("cyan");
  char("b", 60, 31, {
  ...
  color("black");
  text("".concat(abs(ship.nextUp), "m/s"), 76 - (-ship.nextUp > 9 ? 6 : 0), 31);
  char("c", 97, 30);
  color("cyan");
  char("b", 60, 38);
  color("black");
  text("".concat(ship.nextDown, "m/s"), 76 - (ship.nextDown > 9 ? 6 : 0), 38);
  char("c", 97, 37);
}
