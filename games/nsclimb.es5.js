...
function update() {
  ...
  magnets.forEach(function (m) {
    m.pos.y += scr;
    if (m.type !== "") {
      var d = pos.distanceTo(m.pos);
      var a = pos.angleTo(m.pos);
      vel.addWithAngle(a, difficulty / d / d * (m.type === type ? -1 : 1));
    }
    color(m.type === "" ? "light_black" : m.type === "N" ? "red" : "blue");
    var c = box(m.pos, 9, 9).isColliding.rect;
    if (c.blue || c.red) {
      play("hit");
      pos.x = m.pos.x + (m.wallIndex === 0 ? 10 : -10);
      if (m.wallIndex === 0 && vel.x < 0 || m.wallIndex === 1 && vel.x > 0) {
        vel.x *= -0.7;
      }
    }
    color("white");
