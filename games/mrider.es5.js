...
function update() {
  ...
  var scr = rider.pos.x > 50 ? (rider.pos.x - 50) * 0.1 * sqrt(difficulty) : 0;
  rider.pos.x -= scr;
  color("black");
  char(rider.missile == null ? "b" : "a", rider.pos.x, clamp(rider.pos.y, -2, 99), {
  ...
  if (nextMissileTicks < 0) {
    var pos = vec(210, rnd(40, 70));
    missiles.push({
      pos: pos,
      vel: vec(rnd(0.4, 0.5) * sqrt(difficulty)).rotate(pos.angleTo(0, rnd(50, 60)) + PI)
    });
  ...
  nextPlaneTicks--;
  if (nextPlaneTicks < 0) {
    var _pos = vec(203, rnd(30, 90));
    planes.push({
      pos: _pos,
      vel: vec(rnd(0.05, 0.3) * sqrt(difficulty)).rotate(_pos.angleTo(0, rnd(40, 90)) + PI),
      removeTicks: 0,
      baseRemoveTicks: 0
    });
    nextPlaneTicks = rnd(20, 24) / sqrt(difficulty);
  }
  remove(planes, function (p) {
    p.pos.add(p.vel);
    p.pos.x -= scr;
    if (p.removeTicks > 0) {
      color("light_red");
      ...
      if (p.removeTicks <= 0) {
        ...
        multiplier++;
        var rt = p.baseRemoveTicks * 0.9;
        if (rt > 2) {
          planes.forEach(function (ap) {
            if (ap === p) {
              return;
            }
    ...
    color("black");
    var c = char("c", p.pos).isColliding.rect;
    if (c.red) {
      if (rider.missile != null) {
        play("jump");
        particle(p.pos, 9, 3);
