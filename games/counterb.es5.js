...
function update() {
  ...
  var scr = (player.pos.x - player.baseX) * 0.1;
  ...
  color("white");
  rect(scrX, 90, 2, 10);
  var beamSpeed = 4 * sqrt(difficulty);
  ...
  if (counter.pos != null) {
    ...
    if (counter.enemy == null) {
      ...
      counter.radius = clamp(counter.radius + (input.isPressed ? 1 : -2) * sqrt(difficulty), 0, 30);
      if (counter.radius < 1) {
        if (counter.enemy != null) {
          counter.enemy.beamLength = -9999;
        }
  ...
  nextEnemyTicks--;
  if (nextEnemyTicks < 0) {
    var pos = rnd() < 0.6 ? vec(rnd(150, 300), -5) : vec(205, rnd(85));
    var tp = rnd() < 0.05 ? vec(player.pos.x + 9, player.pos.y) : vec(pos.x - rnd(30, 150), player.pos.y);
    enemies.push({
      pos: pos,
  ...
  remove(enemies, function (e) {
    ...
    if (e.beamLength > 0) {
      color("light_red");
      var bp = vec(e.pos).addWithAngle(e.angle, e.beamLength);
      line(e.pos, bp);
      color("red");
      var _c = box(bp, 5).isColliding.rect;
      if (counter.pos == null && _c.cyan) {
        play("powerUp");
        multiplier = 1;
      ...
      if (counter.enemy !== e && (_c.purple || bp.y > 90 || bp.x < -99)) {
        e.beamLength = -9999;
      }
    }
    color("black");
    bar(vec(e.pos).addWithAngle(e.angle + PI / 5 * 4, 3), 1, 2, e.angle);
    var c = bar(e.pos, 2, 3, e.angle).isColliding.rect;
  ...
  if (box(player.pos, 5, 3).isColliding.rect.light_red && player.invincibleTicks < 0 && !(counter.pos != null && counter.pos.distanceTo(player.pos) < counter.radius + 9)) {
    play("explosion");
