...
function update() {
  ...
  roads.forEach(function (r) {
    ...
    if (r.y < py) {
      if (rnd() < 0.1) {
        r.type = currentType === "both" ? rnd() < 0.5 ? "left" : "right" : "both";
        currentType = r.type;
      } else {
        r.type = currentType;
      }
    ...
    if (r.coin !== "none") {
      color("black");
      var _x = r.coin === "left" ? 30 : 70;
      if (char("b", _x, r.y + 5).isColliding["char"].a) {
        play("coin");
  ...
  color("black");
  var c = char("a", x, 90).isColliding.rect;
  ...
  if (lastScore > 0) {
    color("black");
    text("+".concat(lastScore, "x").concat(lastMultiplier), 3, 9);
  }
}
