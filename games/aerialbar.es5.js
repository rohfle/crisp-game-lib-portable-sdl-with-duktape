...
function update() {
  ...
  if (player.bar != null) {
    ...
    if (input.isJustPressed) {
      play("select");
      player.vel.set().addWithAngle(b.angle + PI / 2, (b.angleVel * b.length + player.angleVel * 3) * sqrt(difficulty)).add(0, -sqrt(difficulty) * 0.5);
  ...
  color("cyan");
  if (bar(player.pos, player.length, 4, player.angle, player.center).isColliding.rect.light_cyan && player.vel.y < 0) {
    player.vel.y *= -0.5;
  }
  nextBarDist -= scr;
  if (nextBarDist < 0) {
    var length = rnd(20, 50);
    bars.push({
      x: 110,
      length: length,
      angle: PI / 2 - rnd(PI / 4),
  ...
  remove(bars, function (b) {
    ...
    b.angle += b.angleVel;
    color("black");
    var p = vec(b.x, ceilingY).addWithAngle(b.angle, b.length);
    line(b.x, ceilingY, p);
    color(b.isHeld ? "black" : "blue");
    var c = box(p, 5).isColliding.rect;
    if (c.light_blue) {
      play("explosion");
      color("red");
      text("X", p);
