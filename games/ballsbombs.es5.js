...
function update() {
  ...
  var scr = playerX > 50 ? (playerX - 50) * 0.1 : 0;
  if (explosionX != null) {
    explosionX -= scr;
    explosionXRadius += 1;
  ...
  walkSpeed *= 0.998;
  var c = char(addWithCharCode(isBombPlayer ? "g" : "c", floor(ticks / 20) % 2), playerX, 87).isColliding["char"];
  if (c.e || c.f) {
    play("powerUp");
    isBombPlayer = true;
    bombX = undefined;
  ...
  if (nextBallDist < 0) {
    var pos = vec(rnd() < 0.1 ? rnd(-20, -3) : rnd(153, 200), rnd(10, 40));
    var vel = vec((pos.x < 0 ? 2 : -1) * rnd(0.3, 0.7), 0);
    balls.push({
      pos: pos,
      vel: vel
    });
  ...
  remove(balls, function (b) {
    ...
    var c = char(addWithCharCode("a", b.pos.y > 80 && b.vel.y < 0 ? 1 : 0), b.pos.x, clamp(b.pos.y, 0, 87)).isColliding;
