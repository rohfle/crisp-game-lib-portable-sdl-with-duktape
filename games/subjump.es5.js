...
function update() {
  if (!ticks) {
    points = times(12, function (i) {
      return vec(i * 10 - 10, rnd(75, 85));
    });
    landForm = "sea";
    nextLandFormDist = 50;
    lvy = 0;
  ...
  var gp = points[points.length - 1];
  points.forEach(function (p, i) {
    p.x -= scr;
    var pp = points[wrap(i - 1, 0, 12)];
    if (p.x < -10) {
      if (nextLandFormDist < 0) {
        landForm = landForm === "land" ? "sea" : "land";
        nextLandFormDist = rnd(200, 300) / (landForm === "land" ? 7 / sqrt(difficulty) : 1);
      }
      p.x += 120;
      lvy += rnds(sd) * 2;
      lvy *= 0.95;
  ...
  color("black");
  var c = char(sub.pos.y < 50 ? "a" : addWithCharCode("a", floor(ticks / 7) % 2), sub.pos).isColliding.rect;
  if (c.purple || c.green) {
    play("explosion");
    end();
  }
