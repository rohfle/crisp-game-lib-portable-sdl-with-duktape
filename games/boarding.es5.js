...
function update() {
  ...
  passengers = passengers.filter(function (p) {
    ...
    var c = char(addWithCharCode("a", p.type + 1 + floor(ticks / 30) % 2), p.pos).isColliding["char"];
    if (c.e || c.f) {
      var a = wrap(p.pos.angleTo(p.prevPos), -PI, PI);
      var ra;
      if (c.e) {
      ...
      var v = vec(p.vel.length * 2).rotate(ra);
      p.vel.add(v);
      p.pos.add(p.vel);
      p.pos.add(p.vel);
      p.bc++;
    ...
    if (p.pos.y > gy) {
      var isOk = (p.pos.x - 50) * p.type > 0;
      if (isOk) {
        if (p.bc > 0) {
          play("powerUp");
          addScore(p.bc, p.pos);
          var oy = p.bc * 2 * difficulty;
          tgy += oy;
          bi -= oy;
          if (tgy > 91) {
            tgy = 91;
        ...
        var _oy = sqrt(1 + p.bc) * difficulty;
        if (_oy > 20) {
          _oy = 20;
        }
        tgy -= _oy;
...
function addBoard() {
  var pos = vec(rnd(9, 90), gy - 3);
  if (boards.some(function (b) {
    return b.pos.distanceTo(pos) < 16;
  })) {
    return;
  }
  boards.push({
