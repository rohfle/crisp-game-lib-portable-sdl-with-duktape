...
function update() {
  ...
  var ta = ticks % 10 / 10 * (PI / 4);
  if (ufo.trLength > 1) {
    color("blue");
    times(4, function () {
      line(4 * cos(ta) + ufo.pos.x, ufo.pos.y + 5, 9 * cos(ta) + ufo.pos.x, ufo.pos.y + 5 + ufo.trLength, 2);
      ta += PI / 4;
    });
  }
  color("black");
  ...
  remove(balls, function (b) {
    ...
    balls.forEach(function (ab) {
      ...
      var d = ab.pos.distanceTo(b.pos) - ab.radius - b.radius;
      if (d < 0) {
        b.vel.addWithAngle(ab.pos.angleTo(b.pos), -d / sqrt(b.radius));
      }
    });
    color(b.isRed ? "red" : "black");
    var c = arc(b.pos, b.radius).isColliding;
    ...
    if (b.isRed && !b.isBeamed && (b.pos.x < b.radius || b.pos.x > 99 - b.radius)) {
      play("powerUp");
      b.isRed = false;
    } else if (!b.isBeamed && b.pos.isInRect(10, 12 + b.radius, 80, 5) && b.vel.y > 0) {
      b.isRed = true;
    }
  ...
  if (ufo.trLength > 1) {
    color("cyan");
    times(4, function () {
      line(4 * cos(ta) + ufo.pos.x, ufo.pos.y + 5, 9 * cos(ta) + ufo.pos.x, ufo.pos.y + 5 + ufo.trLength);
      ta += PI / 4;
    });
  }
}
