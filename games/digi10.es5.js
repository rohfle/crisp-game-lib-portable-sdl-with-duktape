...
var addTicks;
var fallNum, fallX;
var isErasing, eraseFrom, eraseTo, eraseTicks;
var eraseMsg;
var multi;
function update() {
  if (ticks === 0) {
  ...
  for (var i = 1; i <= 9; i++) {
    if (fallNum < 0 && (keyboard.code["Digit".concat(i)].isJustPressed || keyboard.code["Numpad".concat(i)].isJustPressed)) {
      fallNum = i;
      fallX = -1;
      digits.push(rndi(1, 10));
    }
  ...
  digits.forEach(function (d, i) {
    var p = vec(99 - (digits.length - i) * 6, 15);
...
function checkTen() {
  ...
  for (var i = 0; i < digits.length; i++) {
    ...
    if (s % 10 === 0) {
      ...
      eraseTicks = 60;
      eraseMsg = "";
      for (var j = eraseFrom; j <= eraseTo; j++) {
        eraseMsg += "".concat(digits[j]).concat(j < eraseTo ? "+" : "=");
      }
      eraseMsg += s;
      if (multi > 1) {
        eraseMsg += "x".concat(multi);
      }
      eraseMsg = eraseMsg.substring(eraseMsg.length - 16);
      addScore(s * multi);
      multi++;
      for (var _j = eraseFrom; _j <= eraseTo - 2; _j++) {
        digits[_j] = -1;
      }
