...
function update() {
  ...
  var sd = difficulty;
  var tc = player.pos.angleTo(50, 50);
  ...
  color("black");
  remove(humans, function (h) {
    var ta;
    if (enemies.length > 0) {
      var ne = getNearestActor(enemies, h.pos);
    ...
    h.ticks += sd;
    var c = char(addWithCharCode("a", floor(h.ticks / 30) % 2), h.pos, {
      mirror: {
        x: h.pos.x > px ? 1 : -1
      }
    });
  ...
  nextEnemyTicks -= sd;
  if (nextEnemyTicks < 0) {
    var ep = vec(50, 50).addWithAngle(rnd(PI * 2), 80);
    var ec = rndi(3, 9);
    times(ec, function () {
      enemies.push({
  ...
  remove(enemies, function (e) {
    var px = e.pos.x;
    if (humans.length > 0) {
      var nh = getNearestActor(humans, e.pos);
      e.vel.addWithAngle(e.pos.angleTo(nh.pos), 0.005);
      e.vel.mul(0.95);
      e.pos.add(vec(e.vel).mul(sd));
    }
    e.ticks += sd;
    var c = char(addWithCharCode("c", floor(e.ticks / 30) % 2), e.pos, {
      mirror: {
        x: e.pos.x > px ? 1 : -1
      }
    }).isColliding;
...
function getNearestActor(actors, pos) {
  return actors.reduce(function (a, b) {
