...
function update() {
  if (!ticks) {
    ...
    lockIndex = 0;
    laser = undefined;
    stars = times(50, function () {
      return {
        pos: vec(rnd(99), rnd(-20, 120)),
        z: rnd(0.5, 3)
      };
    });
    multiplier = 1;
  ...
  if (laser != null) {
    ...
    laser.pos.add(vec(laser.vel).div(laser.z).div(1 + abs(vz) * 9));
    laser.posHistory.push(vec(laser.pos));
    if (laser.posHistory.length > 19) {
      laser.posHistory.shift();
    }
    var pp;
    laser.posHistory.forEach(function (p, i) {
      if (pp != null) {
        color(rnd() < i / 9 ? "purple" : "black");
        var rs = 5 / laser.z;
        line(pp.x + rnds(rs), pp.y + rnds(rs), p.x + rnds(rs), p.y + rnds(rs), 3 / laser.z);
      }
      pp = p;
    });
    var o = 99 / laser.z;
  ...
  if (nextEnemyTicks < 0) {
    var c = rndi(3, 6);
    var vel = vec(0, rnds(0.2, 0.3)).mul(sqrt(difficulty));
    var pos = vec(rnd(10, 90), vel.y > 0 ? -5 : 105);
    var z = rnd(0.5, 3);
    vel.div(sqrt(z));
    times(c, function () {
      enemies.push({
  ...
  remove(enemies, function (e) {
    ...
    if (e.lockIndex >= 0) {
      ...
      var a = -PI / 2 - e.lockTicks * 0.3;
      var r = 5 + e.lockTicks * 5;
      times(4, function (i) {
        var pa = a;
    ...
    if (e.lockIndex < 0 && (e.vel.y < 0 ? e.pos.y < 0 : e.pos.y > 99)) {
      color("red");
      text("X", e.pos.x, clamp(e.pos.y, 5, 95), {
        scale: {
          x: 1.5,
          y: 1.5
        }
...
var bgm = ["@synth@s734516504 v40 l4. o4 r16gr16 gr16>c r16<f+ r16g2.^8. r16 >a+ r16g", "@select@s339235601 v25 l16 o4 c+ccccccc c<g>ccdcdc cccccccc cc+8c+8c+rc+ r<g>cccccc c<g>c<ggg>cc <g+a+a+a+a+a+a+r g+ggg>cr<g>c", "@hit@d@s527705593 v50 l16 o4 cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8.", "@click@d@s404522195 v50 l16 o4 rcr4^16c rcr4.^16 cr4^16c rcr4.^16 cr4^16c rcr4.^16 cr4^16c rcr4.", "@explosion@d@s586342200 v40 l16 o4 r8cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr8. cr8.cr"];
