...
function update() {
  ...
  if (nextFloorDist < 0) {
    ...
    var cc = rndi(ceil(h / 10));
    var cy = rnds(h * 0.5) - cc * 10 / 2;
    var cx = x + rnds(w * 0.2);
    times(cc, function () {
  ...
  color("black");
  remove(fans, function (f) {
    f.ticks += f.speed * f.speed;
    f.pos.add(scr);
    var oa = abs(wrap(f.pos.angleTo(ball.pos) - f.angle, -PI, PI));
    var d = f.pos.distanceTo(ball.pos) + 1;
    var fr = f.speed * clamp(PI / 4 - oa, 0, PI / 4) / d * 3;
    ball.vel.addWithAngle(f.angle, fr);
    box(vec(f.pos).addWithAngle(f.angle + PI, 5), 3);
    var c = bar(f.pos, cos(f.ticks * 0.1) * 9, 3, f.angle + PI / 2).isColliding.rect;
