...
function update() {
  ...
  rect(0, 0, 7, 100);
  rect(93, 0, 7, 100);
  var rx = clamp(input.pos.x, 17, 83);
  color("blue");
  box(rx, 5, 20, 5);
  box(rx, 95, 20, 5);
  wakeColor = "light_black";
  ...
  function updateBall() {
    ...
    if (wakeColor === "light_black" && wakeDist >= ballDist) {
      var r = (wakeDist - ballDist) / ballSize;
      ballPos.set(ballWake.pos.x + ballWake.vel.x * (1 - r), ballWake.pos.y + ballWake.vel.y * (1 - r));
      wakeColor = "light_cyan";
    }
    if (wakeColor === "light_cyan" && (ballWake.pos.y < 7 || ballWake.pos.y > 93)) {
      wakeColor = "light_red";
      wakeCount = 5;
    }
    ballWake.pos.add(ballWake.vel);
    color(wakeColor);
    var c = box(ballWake.pos, ballSize).isColliding.rect;
    if (c.light_blue) {
      if (ballWake.pos.x < 50 && ballWake.vel.x < 0 || ballWake.pos.x > 50 && ballWake.vel.x > 0) {
        ballWake.vel.x *= -1;
      }
    }
    if (c.blue && (ballWake.pos.y < 50 && ballWake.vel.y < 0 || ballWake.pos.y > 50 && ballWake.vel.y > 0)) {
      var d = (ballWake.pos.x - rx) / 10;
      if (abs(d) < 0.6) {
        ballWake.vel.y *= -1;
      } else {
        var a = d > 0 ? PI / 4 + (1 - d) : PI / 4 * 3 - (1 + d);
