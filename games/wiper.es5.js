...
function update() {
  ...
  if (wiperAngle > -PI / 5 && wiperVa > 0 || wiperAngle < -PI / 5 * 4 && wiperVa < 0) {
    play("hit");
    wiperVa *= -1;
  }
  var ta = wiperVa > 0 ? -PI / 5 : -PI / 5 * 4;
  if (wiperAngle > -PI / 3) {
    ta = -PI / 5 * 4;
  }
  ...
  if (nextRainTicks < 0) {
    rains.push({
      pos: vec(rnd(10, 90), rnd(20)),
      size: 2
    });
    nextRainTicks = 10 / difficulty;
  }
  remove(rains, function (r) {
    color("transparent");
    var isRemoved = box(r.pos, sqrt(r.size) * 2).isColliding.rect.black;
    var isAbsorbed = false;
    rains.forEach(function (or) {
      if (r.pos.y > or.pos.y && r.pos.distanceTo(or.pos) < sqrt(r.size + or.size) * 2) {
        or.size += r.size;
