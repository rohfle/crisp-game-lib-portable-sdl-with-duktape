...
function update() {
  ...
  if (nextSparkTicks < 0) {
    var velocity = vec(1, 0).rotate(player.angle + PI + rnds(0.2));
    var position = vec(player.position);
    sparks.push({
      position: position,
      velocity: velocity,
  ...
  remove(sparks, function (s) {
    s.position.add(s.velocity);
    color("".concat(s.size < 5 ? "light_" : "").concat(s.color));
    if (s.size < 5) {
