...
function update() {
  ...
  pillars = pillars.filter(function (p) {
    color(p.color);
    if (box(p.x / p.z + 50, p.size.y / 3 / p.z + 60, p.size.x / p.z, p.size.y / p.z).isColliding.text.T) {
      var ty = p.size.y / 3 / p.z + 60 - p.size.y / p.z / 2;
      if (vy > 0) {
        play("laser");
