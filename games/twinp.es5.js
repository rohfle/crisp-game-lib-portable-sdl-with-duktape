...
function update() {
  ...
  var np = vec(pos).addWithAngle(angle, len);
  color("black");
  line(pos, np, 2);
  ...
  objAddTicks--;
  if (objAddTicks < 0) {
    var type = crossAddCount === 0 ? "x" : "o";
    var _pos = vec(50, 50).addWithAngle(rnd(PI * 2), 80);
    objs.push({
      pos: _pos,
      vel: vec().addWithAngle(_pos.angleTo(rnd(20, 80), rnd(20, 80)), rnd(1, difficulty) * 0.2),
      type: type,
      isIn: false
    });
    crossAddCount--;
  ...
  objs = objs.filter(function (o) {
    ...
    if (o.type === "o") {
      ...
      color("purple");
      var c1 = bar(o.pos, 4, 2, PI / 4).isColliding.rect.black;
      var c2 = bar(o.pos, 4, 2, -PI / 4).isColliding.rect.black;
      if (c1 || c2) {
        play("lucky");
        end();
      }
