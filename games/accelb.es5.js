...
function update() {
  ...
  var pmc = playerMissiles.length;
  remove(playerMissiles, function (m) {
    m.pos.add(m.vel);
    ...
    var d = m.pos.distanceTo(m.target);
    ...
    var mv = sqrt(difficulty) / sqrt(d + 9) * (m.ticks < 9 ? 0.1 : m.ticks < 20 ? 3 : 1);
  ...
  var vva = sqrt(difficulty) * 0.0005;
  remove(missiles, function (m) {
    m.pos.addWithAngle(m.angle, m.speed);
    m.pos.x -= scr;
    var ta = m.pos.angleTo(player.pos);
    var oy = wrap(ta - m.angle, -PI, PI);
    m.va += (oy > 0 ? 1 : -1) * vva;
    m.angle = clamp(wrap(m.angle + m.va, 0, PI * 2), PI / 4 * 3, PI / 4 * 5);
    color("black");
