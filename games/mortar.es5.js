...
function update() {
  if (!ticks) {
    enemies = times(5, function () {
      return {
        pos: vec(rnd(49, 60), rnd(-149, -140)),
        vy: 0.1
      };
    });
    nextEnemyTicks = 0;
    ...
    multiplier = 1;
    sightSpeedRatio = 3;
    grounds = times(99, function () {
      return vec(rnd(99), rnd(-300, 100));
    });
  }
  color("light_black");
  ...
  if (explosion.radius != null) {
    ...
    if (mortar.pos.x < 3 && mortar.vx < 0 || mortar.pos.x > 96 && mortar.vx > 0) {
      mortar.vx *= -1;
    }
  ...
  nextEnemyTicks--;
  if (nextEnemyTicks < 0) {
    var vy = rnd(0.5, sqrt(difficulty) * 2) * 0.1;
    var x = rnd(20, 80);
    times(rndi(5, 9), function () {
      enemies.push({
  ...
  remove(enemies, function (e) {
    var c = char("b", e.pos.x, e.pos.y + oy).isColliding.rect;
    if (e.pos.y > 99) {
      play("lucky");
      text("X", e.pos.x, 97);
      endCount++;
    } else {
      var _vy = e.vy * sqrt(120 - maxEnemyY) / 4;
