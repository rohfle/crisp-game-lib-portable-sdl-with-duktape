...
function update() {
  ...
  if (starAppTicks < 0) {
    ...
    s.vel.addWithAngle(s.pos.angleTo(c) + rnds(1), 0.1 + rnd(difficulty - 1) * 0.1);
    stars.push(s);
  ...
  color("black");
  text("+".concat(addingScore), 3, 95);
  function turnTo(a, p, v) {
    var at = pos.angleTo(p);
    var o = wrap(a - at, -PI, PI);
    if (abs(o) < v) {
      return at;
    } else if (o > 0) {
      return a - v;
