...
function update() {
  ...
  remove(lines, function (l) {
    ...
    if (l.ticks > 0) {
      ...
      color("black");
      particle(l.to, 9, 1);
      for (var _i = 0; _i < rndi(1, 4); _i++) {
        lines.push({
          from: vec(l.to),
          to: vec(l.to),
          vel: vec(l.vel).normalize().rotate(rnds(0.7)).mul(rnd(0.3, 1) * sqrt(difficulty)),
  ...
  color("black");
  if (char(addWithCharCode("b", floor(ticks / 10) % 2), player.x, 87, {
    mirror: {
      x: player.vx > 0 ? 1 : -1
    }
  ...
  remove(stars, function (s) {
    ...
    var c = char("a", s.pos).isColliding["char"];
    if (c.b || c.c) {
      play("coin");
