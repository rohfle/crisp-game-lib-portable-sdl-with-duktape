...
function update() {
  ...
  var scroll = vec(speed * playerAngle, speed);
  ...
  slashes = slashes.filter(function (s) {
    color("purple");
    var c = bar(s.pos, s.width, s.isAppearing ? 20 : 4, s.angle).isColliding.rect;
    if (s.isAppearing) {
      if (c.purple) {
        return false;
      }
  ...
  color("black");
  var pa = -(playerAngle * PI) / 4 + PI / 2;
  var c = bar(50, 90, 4, 3, pa, 0.9).isColliding.rect;
  if (c.purple) {
    play("explosion");
    end();
