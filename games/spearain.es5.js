...
function update() {
  ...
  var sr = input.isPressed ? 0.33 : 1;
  objs = objs.filter(function (o) {
    var s = o.speed * speedRatio * difficulty;
    if (o.isBonus) {
      color("yellow");
      text("$", o.pos);
      if (s > 78) {
  ...
  var s = vx / sqrt(speedRatio) * difficulty;
  x += s;
  ...
  anim += abs(s) * 0.1;
  color("black");
  var cl = char(addWithCharCode("b", floor(anim % 2)), x, 87).isColliding;
  if (cl.text["$"]) {
    play("coin");
