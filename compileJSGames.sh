...
if [ "$#" -eq 0 ]; then
    find submodules/crisp-game-lib/docs/ -maxdepth 2 -mindepth 2 -type f -iname "main.js" ! -path '*/_*' ! -path '*-es5/*' -print0 |
        while IFS= read -r -d '' gamepath; do
            compile_game "$gamepath"
        done
else
    for gamename in "$@"; do
        gamepath="submodules/crisp-game-lib/docs/$gamename/main.js"
        if [ -f "$gamepath" ]; then
            compile_game "$gamepath"
        else
